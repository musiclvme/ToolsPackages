...
#include "bitstream_private.h"
#include "bitstream.h" 

#include "g726.h" 
#include "g726_private.h"
...
#define ADPCM_G726_FILE "adpcm_g726"
...
int main(int argc, char *argv[])
{
    ...
    else
    {
        ...
        if ( (test = sf_open_telephony_read(IN_FILE_NAME, 1, &inhandle)) == NULL)
        {
            fprintf(stderr, "    Cannot open audio file '%s'\n", IN_FILE_NAME);
            exit(2);
        }
        if ((sf_open_telephony_write(OUT_FILE_NAME, 1, &outhandle)) == NULL)
        {
            fprintf(stderr, "    Cannot create audio file '%s'\n", OUT_FILE_NAME);
            exit(2);
        }

		if ((adpcmfile = fopen(ADPCM_G726_FILE, "w+")) == NULL)
		{
			fprintf(stderr, "    Failed to open '%s'\n", ADPCM_G726_FILE);
			exit(2);
		}
		printf("inhandle=%p outhandle=%p, test=%p\n", inhandle, outhandle, test);
